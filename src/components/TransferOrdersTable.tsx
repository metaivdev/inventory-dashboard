...
type SortField = "transfer_order_number" | "date" | "quantity_transfer" | "from_location_name" | "to_location_name" | "status";
...
export function TransferOrdersTable({ orders, isLoading }: TransferOrdersTableProps) {
  ...
  const filteredAndSortedOrders = useMemo(() => {
    ...
    if (locationFilter !== "all") {
      result = result.filter(
        (order) =>
          order.from_location_name === locationFilter ||
          order.to_location_name === locationFilter
      );
    }
                ...
                >
                  ...
                  <td className="py-4 px-4">
                    <div className="flex items-center gap-2 text-sm">
                      <span className="text-slate-300 max-w-[120px] truncate" title={order.from_location_name}>
                        {order.from_location_name}
                      </span>
                      <ArrowRight className="w-4 h-4 text-cyan-400 flex-shrink-0" />
                      <span className="text-slate-300 max-w-[120px] truncate" title={order.to_location_name}>
